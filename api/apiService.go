...
func (a *ApiService) getData(c *gin.Context) (interface{}, error) {
	...
	if err != nil {
		log.Printf("DEBUG: Error checking changes: %v", err)
		return "", err
	}

	log.Println("DEBUG: Fetching onlines...")
	onlines, err := a.StatsService.GetOnlines()
	if err != nil {
		log.Printf("DEBUG: Error fetching onlines: %v", err)
		return "", err
	}
...
// ... (rest of the code remains commented out until data["udptunnels"] = udpTunnelConfigs)
	...
	log.Println("DEBUG: getData successful.")
	return data, nil
}
...
// APIHandler struct (assuming its defined elsewhere, e.g., in apiHandler.go)
// For the purpose of this modification, well assume a.ApiService is available.
...
func (a *ApiService) Login(c *gin.Context) {
	...
	if err != nil {
		logger.Infof("Unable to get sessions max age from DB")
	}
